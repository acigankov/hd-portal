            ...
            <div class="row mb-4">
                <div class="col">
                    <?php if (Yii::$app->user->can('admin')): ?>
                        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['/status/create'])?>" role="button"><?= Yii::t('app', 'New status') ?></a>
                    <?php endif; ?>
                </div>
                ...
                <div class="col">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><?= Yii::t('app', 'List of Statuses') ?></h3>
                        ...
                        <div class="card-body table-responsive p-0">
                            <table class="table table-hover text-nowrap">
                                ...
                                <?php foreach ($dataProvider->getModels() as $item):?>
                                    <tr>
                                        <td><?= $item->id ?></td>
                                        <td><?= Html::a(Html::encode($item->name), ['/status/view', 'id' => $item->id]) ?></td>
                                        ...
                                        <td>
                                            ...
                                            if ($item->for_tickets) $entityTypes[] = Yii::t('app', 'Tickets');
                                        ...
                                        <td>
                                            <?php
                                            $badgeClass = $item->status === Status::STATUS_ACTIVE ? 'bg-success' : 'bg-secondary';
                                            ?>
                                            <span class="badge <?= $badgeClass ?>"><?= Html::encode($item->getStatusLabel()) ?></span>
                                        </td>
                                        <td>
                                            <?= Html::a(Yii::t('app', 'View'), Yii::$app->urlManager->createUrl(['status/view', 'id' => $item->id]) , ['class' => 'btn btn-primary btn-sm' , 'role' => 'button']); ?>
                                            <?php if(Yii::$app->user->can('admin')): ?>
                                                <?= Html::a(Yii::t('app', 'Edit'), Yii::$app->urlManager->createUrl(['status/update', 'id' => $item->id]) , ['class' => 'btn btn-primary btn-sm' , 'role' => 'button']); ?>
                                                <?= Html::button(Yii::t('app', 'Delete'), [
                                                    'class' => 'btn btn-danger btn-sm delete-btn',
                                                    'data-id' => $item->id,
                                                    'data-name' => $item->name,
                                                    'data-url' => Yii::$app->urlManager->createUrl(['status/delete', 'id' => $item->id]),
                                                    'data-toggle' => 'modal',
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?= Yii::t('app', 'Confirm deletion of status') ?>:  <span class="fw-bold fs-5" id="deleteUserName"></span>
                </div>
