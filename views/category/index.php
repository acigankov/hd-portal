            ...
            <div class="row mb-4">
                <div class="col">
                    <?php if (Yii::$app->user->can('admin')): ?>
                        <a class="btn btn-primary" href="<?= \yii\helpers\Url::to(['/category/create'])?>" role="button"><?= Yii::t('app', 'New category') ?></a>
                    <?php endif; ?>
                </div>
                ...
                <div class="col">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><?= Yii::t('app', 'List of Categories') ?></h3>
                        ...
                        <div class="card-body table-responsive p-0">
                            <table class="table table-hover text-nowrap">
                                ...
                                <tr>
                                    <th><?= Yii::t('app', 'ID') ?></th>
                                    <th><?= Yii::t('app', 'Icon') ?></th>
                                    <th><?= Yii::t('app', 'Name') ?></th>
                                    <th><?= Yii::t('app', 'Code') ?></th>
                                    <th><?= Yii::t('app', 'Color') ?></th>
                                    <th><?= Yii::t('app', 'Entity Types') ?></th>
                                ...
                                <?php foreach ($dataProvider->getModels() as $item):?>
                                    <tr>
                                        <td><?= $item->id ?></td>
                                        <td><i class="bi <?= $item->getIconClass() ?>"></i></td>
                                        <td><?= Html::a(Html::encode($item->name), ['/category/view', 'id' => $item->id]) ?></td>
                                        <td><?= Html::encode($item->code) ?></td>
                                        <td><span class="badge <?= $item->getColorBadgeClass() ?>"><?= Html::encode($item->color) ?></span></td>
                                        <td>
                                            <?php
                                            $entityTypes = [];
                                        ...
                                        <td>
                                            <?php
                                            $badgeClass = $item->status === Category::STATUS_ACTIVE ? 'bg-success' : 'bg-secondary';
                                            ?>
                                            <span class="badge <?= $badgeClass ?>"><?= Html::encode($item->getStatusLabel()) ?></span>
                                        </td>
                                        <td>
                                            <?= Html::a(Yii::t('app', 'View'), Yii::$app->urlManager->createUrl(['category/view', 'id' => $item->id]) , ['class' => 'btn btn-primary btn-sm' , 'role' => 'button']); ?>
                                            <?php if(Yii::$app->user->can('admin')): ?>
                                                <?= Html::a(Yii::t('app', 'Edit'), Yii::$app->urlManager->createUrl(['category/update', 'id' => $item->id]) , ['class' => 'btn btn-primary btn-sm' , 'role' => 'button']); ?>
                                                <?= Html::button(Yii::t('app', 'Delete'), [
                                                    'class' => 'btn btn-danger btn-sm delete-btn',
                                                    'data-id' => $item->id,
                                                    'data-name' => $item->name,
                                                    'data-url' => Yii::$app->urlManager->createUrl(['category/delete', 'id' => $item->id]),
                                                    'data-toggle' => 'modal',
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?= Yii::t('app', 'Confirm deletion of category') ?>:  <span class="fw-bold fs-5" id="deleteUserName"></span>
                </div>
